...
int main()
{
 ...
 for(int i=0 ; i<n ;i++)
  cin >> a[i].F >> a[i].S, 
  fr[a[i].F] ++, sc[a[i].S] ++, both[{a[i].F, a[i].S}] ++; 
 ...
 for(auto it : fr) ans1 = mul(ans1, Fact[it.S]); 
 for(auto it : sc) ans2 = mul(ans2, Fact[it.S]); 
 ans = add(ans1, ans2);  
 
 int decr = 1; 
 for(auto it : both) 
  decr = mul(decr, Fact[it.S]);
 
 if(srted) ans = sub(ans, decr);
  cout << sub(Fact[n], ans); 
}